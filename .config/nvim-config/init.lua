...
local config = {
  ...
  mappings = {
    ...
    n = {
      ...
      ["<leader>jv"] = { "<cmd>Telescope find_files cwd=app/views/<CR>" },
      ["<leader>jT"] = { "<cmd>Telescope find_files cwd=test/<CR>" },
      ["<leader>rt"] = {
        '<cmd>2TermExec size=20 direction=horizontal dir=. cmd="clear; bundle exec rails test -f -c %"<CR>',
      },
      ...
      -- keymap.set('n', '<leader>rt', '') -- run tests in window
      -- quick save
